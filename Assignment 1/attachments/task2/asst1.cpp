...
static float g_objScale1        = 1.0;       // scale factor for original x
static float g_objScale2       = 1.0;         // scale factor for original y
...
struct ShaderState {
  ...
  GLint h_uVertexScale1;
  GLint h_uVertexScale2;
  GLint h_uTime;
  ...
  ShaderState(const char* vsfn, const char* fsfn) {
    readAndCompileShader(program, vsfn, fsfn);  //readAndCompileShader读取vertice shader和fragment shader文件，将代码给OpenGL，同时将handle给GLSL program

    const GLuint h = program; // short hand

    // Retrieve handles to uniform variables
    h_uVertexScale1 = safe_glGetUniformLocation(h, "uVertexScale1"); //uVertexScale和h_vVertexScale本质上是一个变量，uVertexScale是这个变量在vshader中的名字，h_vVertexScale是这个变量在c++中的名字。  所以这里只是在两个名字之间建立一种连接
      h_uVertexScale2 = safe_glGetUniformLocation(h, "uVertexScale2");
...
struct SquareGeometry {
  ...
  SquareGeometry() {                    //这个正方形被分成了两个三角形，每3个vertex是一个三角形，每一个行是一个vertex。每一个三角形都有3个attribute，分别是位置sqPos, 纹理sqTex, 颜色sqCol。
    static GLfloat sqPos[12] = {        //初始化各个vertex属性的值
      -.5, -.5,
      .5,  .5,
      .5,  -.5,
    ...
    glBindBuffer(GL_ARRAY_BUFFER, posVbo);  //建立一个buffer，命名为GL_ARRAY_BUFFER和posVbo
    glBufferData(                           //将posVbo属性的值传到buffer中
      GL_ARRAY_BUFFER,
      12*sizeof(GLfloat),
      sqPos,
      GL_STATIC_DRAW);
  ...
  void draw(const ShaderState& curSS) {
    int numverts=6;
    safe_glEnableVertexAttribArray(curSS.h_aPosition);          //glEnable和glVertex读取buffer中的数据
...
static void display(void) {
  ...
  safe_glUniform1i(curSS.h_uTexUnit0, 0);           //为curSS.h_uTexUnit0设默认值0
  safe_glUniform1i(curSS.h_uTexUnit1, 1);
//    g_objScale1 = g_objScale2 = 1.0;
  safe_glUniform1f(curSS.h_uVertexScale1, g_objScale1);
    safe_glUniform1f(curSS.h_uVertexScale2,g_objScale2);
...
static void reshape(int w, int h) {
    if (w/h>=1){
        g_objScale1=(float)h/w;
        g_objScale2=1.0;}
    else
    {
        g_objScale2=(float)w/h;
        g_objScale1=1.0;
    }
  g_width = w;
  g_height = h;
  glViewport(0, 0, w, h);  //glViewport确定画图的范围，范围是(0,0)为左下角，(w,h)为右上角的一个正方形区域；windows窗口的左下角定点为(0,0)
  glutPostRedisplay();
}
...
static void initGlutState(int argc, char **argv) {    //相当于初始化一个画图面板
  glutInit(&argc,argv);
  glutInitDisplayMode(GLUT_RGBA|GLUT_DOUBLE|GLUT_DEPTH);   //RGBA是可用颜色，DOUBLE建立了2个buffer，是用来解决画图的闪烁问题，DEPTH是解决远近问题
  glutInitWindowSize(g_width, g_height);      // create a window
  glutCreateWindow("CS 6533: Hello World 2D");    // title the window

  glutDisplayFunc(display);                   // display rendering callback， display为具体的函数名字，每一种函数，处理一种动作
...
static void initShaders() {
  g_shaderStates.resize(g_numShaders);          //g_shaderStates是ShaderState结构体对象的smart point数组
  for (int i = 0; i < g_numShaders; ++i) {      //g_numShaders为要初始化的ShaderState的个数
    if (g_Gl2Compatible)
      g_shaderStates[i].reset(new ShaderState(g_shaderFilesGl2[i][0], g_shaderFilesGl2[i][1])); //new函数初始化一个ShaderState结构体，并返回一个指针，g_shaderStates的指针值就成这个指针的值了
    else
...
static void loadTexture(GLuint texHandle, const char *ppmFilename) {
  ...
  glActiveTexture(GL_TEXTURE0);     //GL_TEXTURE0作为纹理单元
  glBindTexture(GL_TEXTURE_2D, texHandle); //GL_TEXTURE_2D作为这个纹理单元的名字
