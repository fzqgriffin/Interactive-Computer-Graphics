...
static int g_time			   = 0.0;		// time for sine func
...
struct ShaderState {
  ...
  ShaderState(const char* vsfn, const char* fsfn) {
    ...
    h_uVertexScale = safe_glGetUniformLocation(h, "uVertexScale"); //uVertexScale和h_vVertexScale本质上是一个变量，uVertexScale是这个变量在vshader中的名字，h_vVertexScale是这个变量在c++中的名字。  所以这里只是在两个名字之间建立一种连接
      
      h_uVisible = safe_glGetUniformLocation(h, "uVisible"); //!!!!!!
...
static void display(void) {
  ...
  safe_glUniform1f(curSS.h_uVertexScale, g_objScale);
    safe_glUniform1f(curSS.h_uVisible, g_visible);
  // =======================================
  // TODO: 
  ...
  clock_t t = clock();   //返回从“开启这个程序进程”到“程序中调用C++ clock()函数”时之间的CPU时钟计时单元（clock tick）数
  g_time = (int) t;
  ...
  // between images
  // =========================================
    printf("g_time %d\n",g_time);          // using sin function to let the visibility of the two picture change from 0 to 1 to 0;
    g_time=((int)(g_time/500));
    g_time=g_time%180;                      //only integers can be applied %
        printf("angle %d\n",g_time);
    g_visible=sin(float(g_time)/180 * M_PI);
        printf("sin %f\n",g_visible);
