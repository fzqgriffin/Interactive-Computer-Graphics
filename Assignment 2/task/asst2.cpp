...
static Matrix4 g_skyRbt = Matrix4::makeTranslation(Cvec3(0.0, 0.25, 4.0)); //define a
// ============================================
...
// ============================================
// My code
static Matrix4 g_objectRbt[2] = {Matrix4::makeTranslation(Cvec3(-1,0.5,0)),Matrix4::makeTranslation(Cvec3(1,0.5,0))};  //currently  2 obj are defined
static Matrix4 groundRbt = Matrix4();  // identity
//==============
...
static void motion(const int x, const int y) {
  ...
  if (g_mouseLClickButton && !g_mouseRClickButton) { // left button down?
    m = Matrix4::makeXRotation(-dy) * Matrix4::makeYRotation(dx);                     //mouse go right, make positive y rotation
  }
  else if (g_mouseRClickButton && !g_mouseLClickButton) { // right button down?
    m = Matrix4::makeTranslation(Cvec3(dx, dy, 0) * 0.01);                             // mouse go down, itmes go up
  }
    ...
    if (g_mouseClickDown &&  Count_view == 0 ) {                      // view is sky_camera, world-sky frame
        if (Count_object == 0) {                                            //object is the world
            if (Count_sky_frame == 0) {                                     //  center point is the world
                ...
                Matrix4 relative_frame = Matrix4() * linFact(g_skyRbt);
            ...
            if (Count_sky_frame == 1) {                                     // sky-sky frame
                ...
                Matrix4 relative_frame = transFact(g_skyRbt) * linFact(g_skyRbt);
                g_skyRbt = relative_frame * m * inv(relative_frame) * g_skyRbt;
                eyeRbt = g_skyRbt;
                invEyeRbt = inv(eyeRbt);
                glutPostRedisplay();
        ...
        if (Count_object == 1) {                                     // object is cube 1
            Matrix4 relative_frame = transFact(g_objectRbt[0]) * linFact(g_skyRbt);
        ...
        if (Count_object == 2) {                                     // object is cube 2
            Matrix4 relative_frame = transFact(g_objectRbt[1]) * linFact(g_skyRbt);
    ...
    if (g_mouseClickDown && Count_view == 1 ) {                   // view is cube 1
        ...
        if (Count_object == 1) {                                     // object is cube 1
            ...
            Matrix4 relative_frame = transFact(g_objectRbt[0]) * linFact(g_objectRbt[0]);
            g_objectRbt[0] = relative_frame * m * inv(relative_frame) * g_objectRbt[0];
        ...
        if (Count_object == 2) {                                     // object is cube 2
            Matrix4 relative_frame = transFact(g_objectRbt[1]) * linFact(g_objectRbt[0]);
            g_objectRbt[1] = relative_frame * m * inv(relative_frame) * g_objectRbt[1];
            eyeRbt = g_objectRbt[0];
            invEyeRbt = inv(eyeRbt);
            glutPostRedisplay();
    ...
    if (g_mouseClickDown && Count_view == 2 ) {                   // view is cube2
        ...
        if (Count_object == 1) {                                     // object is cube 1
            Matrix4 relative_frame = transFact(g_objectRbt[0]) * linFact(g_objectRbt[1]);
            g_objectRbt[0] = relative_frame * m * inv(relative_frame) * g_objectRbt[0];
        ...
        if (Count_object == 2) {                                     // object is cube 2
            ...
            Matrix4 relative_frame = transFact(g_objectRbt[1]) * linFact(g_objectRbt[1]);
            g_objectRbt[1] = relative_frame * m * inv(relative_frame) * g_objectRbt[1];
            eyeRbt = g_objectRbt[1];
            invEyeRbt = inv(eyeRbt);
            glutPostRedisplay();
...
static void reset()
{
    ...
    g_objectRbt[0] = Matrix4::makeTranslation(Cvec3(-1,0.5,0));
    g_objectRbt[1] = Matrix4::makeTranslation(Cvec3(1,0.5,0));
    groundRbt =  Matrix4();
    eyeRbt = g_skyRbt;
    invEyeRbt = inv(eyeRbt);
    Count_view = 0;
